...
# Make Strands optional since it may not be available in all environments
try:
...
class BaseStrandsAgent(ABC):
    ...
    Note: Requires Strands dependencies to be installed for full functionality.
    """
    ...
    def __init__(self, config: Optional[Any] = None):
        ...
        Raises:
            ImportError: If Strands dependencies are not available
        """
        if not STRANDS_AVAILABLE:
            raise ImportError(
                "Strands dependencies not available. Install them to use BaseStrandsAgent:\n"
                "pip install strands"
            )
    ...
    def _initialize_mcp_and_agent(self):
        ...
        if not STRANDS_AVAILABLE:
            raise RuntimeError("Strands dependencies not available")
    ...
    def _create_strands_agent(self, tools: List[Any]) -> Agent:
        ...
        try:
            ...
            if bedrock_available and hasattr(model_config, '__class__'):
                # Check if model_config is a BedrockModel instance
                model_class_name = model_config.__class__.__name__
                if 'BedrockModel' in model_class_name:
    ...
    async def stream_chat(self, message: str):
        ...
        """
        if not STRANDS_AVAILABLE:
            yield {"error": "Strands dependencies not available"}
            return
    ...
    def chat(self, message: str) -> Dict[str, Any]:
        ...
        if not STRANDS_AVAILABLE:
            return {
                "answer": "Strands dependencies not available",
                "metadata": {
                    "error": True,
                    "error_message": "Strands dependencies not available"
                }
            }
